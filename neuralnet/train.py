...
from tokenizers import Tokenizer,  create_tokenizer, tokenize_and_pad_training_data, save_tokenizer
...
Seq2SeqModel.compile(optimizer=optimizer, loss='sparse_categorical_crossentropy', metrics=['accuracy'])
Seq2SeqModel.summary()


def train(encoder_input_data, decoder_input_data, decoder_output_data, epochs, batch_size, verbose, save_at):
    with tf.device('/device:GPU:0' if utils.check_cuda else '/cpu:0'):
        for epoch in range(1, epochs+1):
            print(f'Epochs {epoch}/{epochs}')
            Seq2SeqModel.fit([encoder_input_data, decoder_input_data], decoder_output_data, batch_size=batch_size, epochs=1, verbose=verbose)
            if epoch % SAVE_AT == 0:
                Seq2SeqModel.save_weights(f'seq2seq-weights-{epochs}-epochs-{LEARNING_RATE}-learning_rate.h5')


train(encoder_input_data, decoder_input_data, decoder_output_data, EPOCHS, BATCH_SIZE, VERBOSE, SAVE_AT)
